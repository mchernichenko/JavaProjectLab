package org.billing.jlab.patterns.composite.menu;

/**
 * Создание древовидной структуры меню, которе в свою очередь, помимо элементов меню может содержать подменю
 */

public class MenuTestDrive {
	public static void main(String args[]) {

		// Создание, точнее объявление различных меню. Т.е эти меню пока без содержания, просто объявления.
		...
		// Добавляем в меню подменю, т.е. говорим что весь ассортимент блюд сгруппирован по 3 основным подменю
		allMenus.add(pancakeHouseMenu);
		allMenus.add(dinerMenu);
		allMenus.add(cafeMenu);
		// некоторые подменю в свою очередь будут содержать вложенное подменю
		dinerMenu.add(dessertMenu);
		cafeMenu.add(coffeeMenu);

		// НАПОЛНЕНИЕ КОНКРЕТНЫХ МЕНЮ КОНКРЕТНВМИ БЛЮДАМИ

		// создаём конкретные элементы меню pancakeHouseMenu и сразу их добавляем в меню pancakeHouseMenu
		...
		// добавляем меню в клиента-потребителя меню, в данном случае официантка и печатаем его
