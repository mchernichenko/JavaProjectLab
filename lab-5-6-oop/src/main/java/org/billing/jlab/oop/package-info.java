 ...
 *      3.1 Comparable<T> - реализация этого интерфейса гарантирует, что объекты могут сравниваться, сортоваться
 ...
 *
 *    4. В интерфейсе можно объявлять переменные, при этом она должна ссылаться на объект класса, реализующий данный интерфейс
 *       и можно объявлять константы, например, как простое объявление переменной int LIMIT = 95,
 *       но они автоматически являются public static final, хотя писать это не обязательно
 *    5. Статических методов нельзя
 */
