 ...
 *      2. protected - видимость внутри пакета и в наследниках класса, т.е. применяется когда требуется доступ к полям/методам непосредственно из подклассов.
 *      Важно: вот в наследниках класса protected-члены родительского класса можно увидеть только если мы используем this или super.
 *             Если же мы обращаемся к переменной-ссылке на экземпляр родительского класса, то к его protected-членам доступа не будет.
 *             Если классы в одном пакете, то без разницы указан protected или можификатора вообще нет.
 *                в том же пакете можно получить доступ к protected членам через ссылку на объект, а вот в другом пакете уже не прокатит такая штука, только через наследников
 *             Если в разных пакетах, то достут полько в наследниках.
 *
 *      Например: Если B extends A, то дополнительно в экземпляре В доступны protected-члены родительского класса этого же экземпляра
 *      (а не любого экземпляра родительского класса , который можно создать new A().runProtectedMethod();
 *      к так нельзя обращаться - к переменной-ссылке на экземпляр родительского класса).
 *
 ...
 *
 * ИТОГО: Если у нас есть класс B, то
 * - при обращении к экземпяру А из методов класса B будет доступ только к public членам и,
 * - если B в одном пакете с A, то к protected и default.
 * - если B extends A, то дополнительно в экземпляре В доступны protected-члены родительского класса этого же экземпляра (а не любого экземпляра родительского класса).
 ...
 Если у нас есть экземпляр А, то при обращении к другому экземпяру А будет доступ ко всем членам.
 *
 ...
 *  - final - предотвращает наследование методов всего класса или отдельного класса.
 ...
 *
 *    4. В интерфейсе можно объявлять переменные. Они автоматически будут иметь модификатор public static final  т.е. автоматически становятся константами, например,
 ...
 *    5. Интерфейс может иметь статические методы, и для их вывова нельзя использовать ссылочные переменные, в отличие от статических методов класса
 *
 *  *** АБСТРАКЦИИ ***
 * 1. Абстрактный класс - единственная его цель это быть расширенным (т.е. он не может быть final).
 * 2. Создать экземпляр абстрактного класса нельзя => допускается отсутствие реализоции некоторых методов
 * 3. Абстрактный класс может не содержать ни одного абстрактного метода!
 * 4. Если класс имеет хотябы один абстратный класс, то он должен быть помечен модификатором abstract
 * 5. Класс который наследуется от аббстрактного должен, либо раелизовать абстрактные методы, либо быть абстрактный
 * 6. Класс который реализует интерфейс, либо должен реализовать все его методы, либо быть абстрактным.
 * 7. Абстратный метод заканчивается ; и не имеет тела
 *
