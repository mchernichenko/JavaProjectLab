// Пакеты служат в основном для обеспечения однозначности имен классов, а также облегчают организацию работы с классами.
 ...
 * Статические поля: инициализируются при первичной загрузке класса JVM. Все операторы, задающие начальные значения статич. полей, а также статич. блоки инициализации
  выполняются в порядке их перечисления в объявлении класса. Если поле явно не проинициализировано, то по дефолту приймет значение 0/false/null.
 ...
 * Пакеты и classpath:
 Иерархия файловой системы должна отражать иерархию пакета Java, а classpath указывает на базовые каталоги в которых могут содержаться файлы классов, т.е. какие директории в файловой системе являются корневыми для иерархии пакета Java.
 jre/lib и jre/lib/ext всегда по дефолту включены в classpath, поэтому лучше там ничего не размещать
 ...
 - если пекет для класса не указан, то в качастве возможных источников рассматриваются пакеты,указанные в директиве import
 ...
 * Импорт:
 ...
 - Импорт со "*" можно импользовать для импорта классов только одного пакета. Для лучшей читаемости рекомендуется импортировать конкретные классы, чтобы сразу видеть какие классы используются.
 ...
 - Cтатический импорт позволяет импортировать статические методы и поля и образаться к ним без указания классов (например, out.println(), но это не всегда наглядно, так что сомнительная польза)
 - java.lang импортируется по дефолту всегда
